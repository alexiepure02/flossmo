...
window.selectCallback = function (variant, selector) {
  ...
  if (variant && variant.available === true) {
    ...
    $('.add_to_cart', $product).removeClass('disabled').removeAttr('disabled').find('span.text')
...
window.productPage = {
  ...
  init() {
    ...
    $('.js-product_sections .product_form_options').each(function () {
      ...
      if ($(this).find('.selector-wrapper select').length != $(this).data('options-size')) {
        new window.Shopify.OptionSelectors(
          $(this).data('select-id'), { 
            product: $(this).data('product'), 
            onVariantSelected: (variant, selector) => {
              currentVariant = variant;
              
              $('.price_box').html(`<span class="money">${window.Shopify.formatMoney(currentVariant.compare_at_price, $('body').data('money-format'))} </span><span class="savings-details">${window.Shopify.formatMoney(currentVariant.price, $('body').data('money-format'))}</span>`);
                ...
                handler: (priceUIFragment, _product, { variant: v }) => {
                  if (isPriceSavingsEnabled && v) {
                    if (v.compare_at_price > v.price) {
                      ...
                      span1.classList.add('sale', 'savings');
                      const savings1 = v.compare_at_price - v.price;
                      const percentage1 = Math.round((savings1 / v.compare_at_price) * 100);
                      span1.innerHTML = `<strong>You're saving ${percentage}% </strong>`;
      				        $('.custom--info-disc .price-sec').html(span1);


                      const span2 = document.createElement('span');
                      span2.classList.add('saving-amount');
                      var str = v.title;

                      var item1 = "2";
                      var item2 = "3";

                      if( str.includes(item1) ){
                        span2.innerHTML = `<span class="savings-details">Only ${window.Shopify.formatMoney(v.price/2, $('body').data('money-format'))} /each</span><span class="money">${window.Shopify.formatMoney( v.compare_at_price/2, $('body').data('money-format'))}</span>`;
                      } else if( str.includes(item2) ) {
                        span2.innerHTML = `<span class="savings-details">Only ${window.Shopify.formatMoney(v.price/3, $('body').data('money-format'))} /each</span><span class="money">${window.Shopify.formatMoney( v.compare_at_price/3, $('body').data('money-format'))}</span>`;
                      } else{
                        span2.innerHTML = `<span class="savings-details">Only ${window.Shopify.formatMoney(v.price, $('body').data('money-format'))} /each</span><span class="money">${window.Shopify.formatMoney(v.compare_at_price, $('body').data('money-format'))}</span>`;
                      }

      				        $('.custom--info-disc .saving-sec').html(span2);

                      $('.price_box').html(`<span class="money">${window.Shopify.formatMoney(v.compare_at_price, $('body').data('money-format'))} </span><span class="savings-details">${window.Shopify.formatMoney(v.price, $('body').data('money-format'))}</span>`);
                    }
                  }
  ...
  productSwatches() {
    ...
    if (window.PXUTheme.theme_settings.product_form_style === 'radio') {
      ...
      if ($('.js-product_sections').length) {
        const $productForms = $('.js-product_sections').find('.product_form');
        ...
        $productForms.each((_, productForm) => {
          ...
          const JSONData = $productForm.data('product');
          const productID = $productForm.data('product-id');
          const productSection = `.product-${productID} .js-product_sections`;
          const swatchOptions = $productForm.find('.swatch_options .swatchs');
          if (swatchOptions.length > 1) {
            window.Shopify.linkOptionSelectors(JSONData, productSection);
          }
      ...
      // Add click event when there is more than one product on the page (eg. Collection in Detail)
      if ($('.js-product_sections').length > 1) {
        $('body').on('click', '.swatch-elements', swatchElement => {
          const $swatchElement = $(swatchElement.currentTarget);
          const swatchValue = $swatchElement.data('value').toString();
